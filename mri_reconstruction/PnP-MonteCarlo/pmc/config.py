...
from pmc.utils.memory_logger import CUDAMemoryLogger

def unknown_to_merge_list(unknown):
    """
    Convert CLI overrides like:
      ["--model.estimator.batch_size", "16", "--model.gamma", "5e-6"]
    into yacs/fvcore merge list:
      ["model.estimator.batch_size", "16", "model.gamma", "5e-6"]
    """
    if not unknown:
        return []
    ...
    if len(unknown) % 2 != 0:
        raise ValueError(
            f"Overrides must be key/value pairs. Got odd-length list: {unknown}"
        )

    out = []
    for k, v in zip(unknown[0::2], unknown[1::2]):
        if not k.startswith("--"):
            raise ValueError(f"Override key must start with '--'. Got: {k}")
        out.extend([k[2:], v])  # strip leading "--"
    return out
...
class Configurator:
    def __init__(self, args):
        ...
        # Apply CLI overrides BEFORE default_fix/freeze
        if hasattr(args, "overrides") and args.overrides:
            self.cfg.merge_from_list(args.overrides)
    ...
    def _init_model(self):
        ...
        # initialize the estimator (change the estimator module if needed)
        noise_type = self.cfg.model.estimator.noise_type
        assert noise_type in ('coil', 'noise', 'zo'), f"Possible noise types are {noise_type}."
        print("noise_type:", noise_type)
        est_ob = self.str_to_class(f"pmc.estimators_{noise_type}", self.cfg.model.estimator.name)
        init_dict = self.cfg_to_dict_without_sth(self.cfg.model.estimator, ['forward_model'])
        estimator = est_ob(forward_model=forward_model, **init_dict)
        ...
        mem_logger = self.init_params_without_name("pmc.utils.memory_logger", self.cfg.model.mem_logger)

        # initialize model
        model_ob = self.str_to_class('pmc.algorithms', self.cfg.model.name)
        init_dict = self.cfg_to_dict_without_sth(self.cfg.model, ['name', 'forward_model', 'score_fn', 'coeff', 'transform', 'estimator', 'mem_logger'])
        return model_ob(forward_model, estimator, score_fn, coeff, transform=transform, mem_logger=mem_logger, **init_dict)

    def _init_dataloader(self):
        self.cfg.defrost()
        self.cfg.dataset.data_used = self.cfg.inference.inference_args.max_num_batches
        self.cfg.freeze()
