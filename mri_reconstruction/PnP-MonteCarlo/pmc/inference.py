...
class PMCInference(ABC):
    ...
    def sample(
        ...
        # initialize previous estimate and iterates as 'None'
        df_grad_est, xprev = None, None
        ...
        for t in pbar:
            ...
            iteration_outputs = self.pmc(xcurr, y, t, df_grad_est=df_grad_est, xprev=xprev)
            xnext, xnext_drift, _, drift, _, _, df_grad_est, xprev = iteration_outputs
    ...
    def __call__(
        ...
        for batch_idx, x in tqdm(enumerate(dataloader), total=max_num_batches, desc='batch'):
            ...
            add_stats = {
                'runtime_s': torch.tensor(runtime_sec),
                'peak_mem_gb': torch.tensor(peak_mem_gb), 
                'peak_mem_bytes': torch.tensor(peak_mem_bytes)
            }
            ...
            if self.callbacks is not None:
                for callback in self.callbacks:
                    callback.on_batch_end(
                        self, (xrecons, xdrift_recons), (xrecon_mean, xdrift_recon_mean),
                        (xrecon_std, xdrift_recon_std), batch, batch_idx, add_info=add_stats)
