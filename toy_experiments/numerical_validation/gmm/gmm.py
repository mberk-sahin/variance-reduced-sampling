...
from gmm.utils import calculate_matmul, calculate_matmul_n_times

# This code was taken from https://github.com/ldeecke/gmm-torch
...
class GaussianMixture(torch.nn.Module):
    ...
    def _init_params(self):
        if self.mu_init is not None:
            ...
            self.mu = torch.nn.Parameter(torch.randn(1, self.n_components, self.n_features, generator=self.torch_rng), requires_grad=False)
    ...
    def sample(self, n):
        ...
        for k in np.arange(self.n_components)[counts > 0]: 
            if self.covariance_type == "diag":
                x_k = self.mu[0, k] + torch.randn(int(counts[k]), self.n_features, device=x.device, generator=self.torch_rng) * torch.sqrt(self.var[0, k])
            elif self.covariance_type == "full":
    ...
    def get_kmeans_mu(self, x, n_centers, init_times=50, min_delta=1e-3):
        ...
        for i in range(init_times):
            tmp_center = x[self.np_rng.choice(np.arange(x.shape[0]), size=n_centers, replace=False), ...]
